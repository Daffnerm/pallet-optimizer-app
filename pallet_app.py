...
def calculate_capacity(l, w, h, max_h):
    ...
    orient1 = (PALLET_LENGTH // l) * (PALET_WIDTH // w)
    orient2 = (PALLET_LENGTH // w) * (PALET_WIDTH // l)
    theoretical_max = max(orient1, orient2)
    
    # Mixed-orientation attempt with rectpack
    packer = newPacker()
    packer.add_bin(PALLET_LENGTH, PALET_WIDTH)
...
if carton_height > max_height:
    ...
    ax.set_ylim(0, PALET_WIDTH)
    ax.set_aspect('equal')

    # Draw pallet border
    pallet_border = plt.Rectangle((0, 0), PALLET_LENGTH, PALET_WIDTH, 
                                fill=False, edgecolor='black', linewidth=2)
    ax.add_patch(pallet_border)

    if cartons_per_layer > 0:
        # Special visualization for 17×13 cartons - FIXED DIMENSIONS
        if (carton_length == 17 and carton_width == 13) or (carton_length == 13 and carton_width == 17):
            positions = [
                # Bottom row - 2 horizontal (17×13)
                {"x": 0, "y": 0, "w": 17, "h": 13},
                {"x": 17, "y": 0, "w": 17, "h": 13},
                
                # Middle row - 2 horizontal + 1 vertical
                {"x": 0, "y": 13, "w": 17, "h": 13},
                {"x": 17, "y": 13, "w": 13, "h": 17},  # vertical (13×17)
                {"x": 30, "y": 13, "w": 17, "h": 13},
                
                # Top row - 1 vertical + 1 horizontal + 1 vertical
                {"x": 0, "y": 30, "w": 13, "h": 17},  # vertical (13×17)
                {"x": 13, "y": 30, "w": 17, "h": 13},
                {"x": 30, "y": 30, "w": 13, "h": 17}   # vertical (13×17)
            ]
            
            # Draw cartons with different colors for horizontal/vertical
            for i, pos in enumerate(positions):
                is_vertical = pos["h"] > pos["w"]  # Height > Width = vertical
